									...
									{expenses.map((expense) => (
										<div key={expense.id} className="flex items-center gap-1.5">
											...
											<ChfNumberInput
												...
												onChange={(value) => updateExpenseAmount(expense.id, value)}
			...
			<Card className="compact-card border-t-4 border-t-navy">
				<CardHeader className="card-header">
					...
					<CardDescription className="text-xs">
						Your financial future based on current savings
					</CardDescription>
				</CardHeader>
				<CardContent className="card-content">
					<div className="grid gap-4 md:grid-cols-2">
								...
								{[1, 2, 5, 10, 20, 30].map((year) => (
									<div key={year} className="flex items-center justify-between">
										...
										<span className="text-xs font-medium">
											CHF {formatNumber(monthlySavings * 12 * year)}
										</span>
									</div>
								))}
							</div>
						...
						<div className="space-y-3">
									...
									return (
										<div key={year} className="flex items-center justify-between">
											...
											<span className="text-xs font-medium text-navy">
												CHF {formatNumber(investedAmount)}
											</span>
										</div>
									)
								})}
							...
							<div className="text-xs text-muted-foreground pt-1">
								* Assumes monthly contributions of CHF {formatNumber(monthlySavings)} with a 7%
								annual return
							</div>
						</div>
					</div>
